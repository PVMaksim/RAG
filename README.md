...
```bash
cp .env.example .env
# Добавь AI_API_KEY и путь к проектам
...
| Переменная           | Описание |
|------------          |---------|
| `AI_API_KEY`         | Ключ с console.anthropic.com (формат `sk-ant-...`) |
...
Проект использует **Nginx** в качестве обратного прокси-сервера для объединения фронтенда и бэкенда под одним адресом.

| Сервис          | Внутренний порт | Внешний доступ  | Описание                                              |
| :---            | :---            | :---            | :---                                                  |
| **Frontend**    | 3000            | ❌ Закрыт       | Next.js приложение доступно только внутри сети Docker |
| **Backend API** | 8000            | ❌ Закрыт       | FastAPI сервер доступен только внутри сети Docker     |
| **Nginx Proxy** | 80              | ✅ **Открыт**   | Единая точка входа для пользователя                   |

### 🔗 Как открыть приложение

Используйте основной порт Nginx:
👉 ** `http://localhost:80`

> **Почему не порт 3000?**
> Порт 3000 работает только внутри контейнеров. Nginx перенаправляет запросы с порта 80 на фронтенд (порт 3000) и бэкенд (порт 8000), обеспечивая единую точку входа и безопасность.

### 🛠 Для разработчиков

Если вам нужен прямой доступ к сервисам (например, для отладки), вы можете временно изменить `docker-compose.yml`, добавив проброс портов:

```yaml
services:
  frontend:
    ports:
      - "3000:3000" # Раскомментировать для прямого доступа
  backend:
    ports:
      - "8000:8000" # Раскомментировать для прямого доступа